...
deff('[z]=F(x)','z=100*(x(1)^2 - x(2))^2 + (1-x(1))^2');
deff('[z]=gradF(x)','z=[2*(200*x(1)^3 - 200*x(1)*x(2) + x(1) - 1), -200*(x(1)^2 - x(2))]');
deff('[z]=HessF(x)','z=[1200*x(1)^2 - 400*x(2) + 2 , -400*x(1) ; -400*x(1) , 200]');
...
sol4=GradPasOptArmijo(x0,F,gradF,tol,nmax);
plot2d(sol4(1,:),sol4(2,:),style=[5],leg="Armijo");

// gradient pas optimal Golstein
disp("NOUVELLE LIGNE Golstein");
sol5=GradPasOptGoldstein(x0,F,gradF,tol,nmax);
plot2d(sol5(1,:),sol5(2,:),style=[6],leg="Goldstein");
