...
On a deux test d'ajustement : Kolmogorov-Smirnov et $ \mathcal{X}^2 $ \begin{itemize}
    ...
    \item $ \mathcal{X}^2 $ : Test asymptotique, $ n \geq 50 $ au minimum + Condition. fonction dans tous les cas.
\end{itemize}
...
\begin{thm}[].
    \begin{enumerate}
        ...
        \[
            h_{n,m} = \sup _{s \in \mathbb{R}} \left| \frac{1}{n}\sum_{i=1}^{n} \mathbbm{1}_{U_i \leq s} - \frac{1}{n}\sum_{j=1}^{n} \mathbbm{1}_{V_i \leq s} \right| 
        .\]
...
\[
    \sup _{s \in \mathbb{R}} \left| \frac{1}{n}\sum_{i=1}^{n} \mathbbm{1}_{X_i \leq t} - \frac{1}{n}\sum_{j=1}^{n} \mathbbm{1}_{X_i \leq t} \right| 
.\]
    ...
    \begin{enumerate}
        ...
        \item Si $ n $ et $ m $ sont trop grands, on utilise le résultat suivant : \\
            ...
            \begin{table}[!h]
                ...
                \begin{tabular}{|l|l|l|l|l|l|l|}
                \hline
                    $ X_{(1)} $ & $ X_{(2)} $ & $ X_{(1)} $ & $ X_{(3)} $ & $ Y_{(2)} $ & $ X_{(4)} $ & ... \\ \hline
